...
function formulaires_inscription2_charger_dist($id_auteur = NULL,$redirect = null){
	...
	if (is_numeric($id_auteur)) {
	    ...
	    foreach($champs as $clef =>$valeurs) {
            if (_request($valeurs)) {
                if(!is_array(_request($valeurs)))$champs[$valeurs] = trim(_request($valeurs));
                else $champs[$valeurs] = _request($valeurs);
            }
...
function formulaires_inscription2_verifier_dist($id_auteur = null,$redirect = null){
	...
	foreach($champs as $clef => $valeur) {
		...
		if(!is_array(_request($valeur)))$valeurs[$valeur]  = trim(_request($valeur));
        else $valeurs[$valeur]  = implode(',',_request($valeur));      

		// On vérifie s'il est obligatoire et s'il est bien rempli
		if ((lire_config('inscription2/'.$valeur.'_obligatoire') == 'on' ) && ((empty($valeurs[$valeur]) OR (strlen($valeurs[$valeur]) == 0)))) {
...
function formulaires_inscription2_traiter_dist($id_auteur = NULL,$redirect = null){
	...
	*/
	...
	foreach($champs as $clef => $valeur) {
		...
		if(!is_array(_request($valeur)))$valeurs[$valeur] = trim(_request($valeur));
        else $valeurs[$valeur]=serialize(_request($valeur));
		if($valeur == 'naissance'){
			include_spip('inc/date');
			$annee = trim(_request('annee'));
			$mois = _request('mois');
    ...
    if (!$new){
        ...
        if($mode == 'modification_auteur_simple'){
			...
			$notifier('inscription',$id_auteur,'modifie');	
			}
        }
        $editable = true;
    } else {
		...
		$notifier = charger_fonction("notifications",'inc',true);
		if (function_exists($notifier)) {
			$notifier('inscription',$id_auteur,'new');	
		}
    }
